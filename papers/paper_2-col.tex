...
Figure \ref{fig:1} shows the training result of the above algorithm (300
moving average reward vs iteration), with 600 iterations (5 episodes per
iteration), \(T=300\), \(\gamma=0.99\),
\(\alpha_\theta=\alpha_{\bm{w}}=\num{3e-4}\).
\begin{figure*}
    \centering\includegraphics[scale=0.67]{a2c_out}
    \caption{Training Result for Algorithm a2c\label{fig:1}}
\end{figure*}
...
\For{iteration \(k=0, 1, 2, ...\)}
    ...
    \For{\(t=0, 1, ..., T\) in each trajectory \(\tau\) in \(\mathcal{D}_k\)}
...
\vspace{3mm}\newline Figure \ref{fig:2} shows the training result of these
...
\begin{figure*}
    \centering\includegraphics[scale=0.67]{out}
    \caption{Training Result Comparison for Three Algorithm\label{fig:2}}
\end{figure*}
As is shown in Figure \ref{fig:2}, the Actor-Critic algorithm converges the fastest,
