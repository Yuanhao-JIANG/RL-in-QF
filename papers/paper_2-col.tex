...
\keywords{
    Reinforcement Learning, Artificial Neural Networks, Quantitative Finance
}
...
we model the interaction between agent and environment using MDP, and the 
mathematics behind those methods to solve the reinforcement learning problem, 
especially the Actor-Critic algorithm.
Then we start building the environment, and use ANNs as our pricing models, 
and writing algorithms that are specific to our financial problem. 
In the end, we will compare and discuss different algorithms in terms of their
rate of convergence to a stable reward.
\end{abstract}
