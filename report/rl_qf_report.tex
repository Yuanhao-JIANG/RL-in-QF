...
\[S_t\rightarrow\underset{\text{ANN for value function}}{
        \boxed{
            \underset{len(S)\rightarrow 128}{\text{Linear}}
            \rightarrow\text{ReLU}
            \rightarrow\underset{128\rightarrow 256}{\text{Linear}}
            \rightarrow\text{ReLU}
            \rightarrow\underset{256\rightarrow 128}{\text{Linear}}
            \rightarrow\text{ReLU}
...
}\rightarrow \text{Softmax}\rightarrow c_t\]
where \(len(S)\) denotes the vector length of the state and \(n(A)\) denotes the number of actions.
...
\Loop \hspace{0.3mm} (for each episode)
    ...
    \State Initialize \(S\) (first state of the episode)
    \State \(I \gets 1\)
    \Loop \hspace{0.3mm} (for each time step, \(N\) in total)
